...
import { readStores, writeStores } from '../lib/store-manager.js';
...
export default class BulkAdd extends Command {
    ...
    async run() {
        ...
        // Detect file format
        const fileFormat = flags.format || (args.file.endsWith('.json') ? 'json' : 'csv');
        ...
        for (let i = 0; i < merchants.length; i++) {
            ...
            try {
                ...
                if (!input.name || input.name.trim().length === 0) {
                    results.failed++;
                    results.errors.push(`Row ${num}: Merchant name cannot be empty`);
                    continue;
                }
                ...
                if (!/^[a-z0-9-]+$/.test(trimmedHandle)) {
                    results.failed++;
                    results.errors.push(`Row ${num}: Invalid handle format "${input.handle}" (must be lowercase letters, numbers, and hyphens)`);
                    continue;
                }
                // Check for duplicates
                if (existingHandles.has(trimmedHandle)) {
                    this.log(`[${num}/${total}] ⚠ Skipped "${input.name}" - handle "${trimmedHandle}" already exists`);
                    results.skipped++;
                    continue;
                }
                ...
                if (!flags['skip-validation']) {
                    this.log(`[${num}/${total}] Checking "${input.name}" (${trimmedHandle})...`);
                    try {
                        const storeInfo = await validateAndGetStoreInfo(trimmedHandle);
                        if (!storeInfo.exists) {
                            results.failed++;
                            results.errors.push(`Row ${num}: Store "${trimmedHandle}.myshopify.com" not found`);
                            continue;
                        }
                        ...
                        results.failed++;
                        results.errors.push(`Row ${num}: Failed to validate store - ${error.message}`);
                        continue;
                    }
                }
                // Create merchant
                ...
                this.log(`[${num}/${total}] ✓ Added "${merchant.name}" (${merchant.handle})`);
                results.added++;
            }
            catch (error) {
                results.failed++;
        ...
        try {
            ...
            return data.map((item) => {
                if (!item.name || !item.handle) {
                    throw new Error('Each JSON object must have "name" and "handle" fields');
                }
                return {
    ...
    parseCSV(content) {
        ...
        return lines.slice(startIndex).map((line, index) => {
            ...
            if (parts.length < 2) {
                throw new Error(`Line ${startIndex + index + 1}: CSV must have at least 2 columns (name, handle)`);
            }
            return {
                name: parts[0],
                handle: parts[1],
...
//# sourceMappingURL=bulk-add.js.map