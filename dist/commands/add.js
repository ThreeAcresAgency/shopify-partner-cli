...
export default class Add extends Command {
    ...
    async run() {
        ...
        if (!/^[a-z0-9-]+$/.test(trimmedHandle)) {
            this.error('Handle must contain only lowercase letters, numbers, and hyphens');
        }
        ...
        if (!storeInfo.exists) {
            this.error(`Store "${trimmedHandle}.myshopify.com" not found or not accessible. Please verify the handle is correct.`);
        }
        // Check if merchant with same handle already exists
        const existingMerchants = await readStores();
        const duplicate = existingMerchants.find((m) => m.handle === trimmedHandle);
        if (duplicate) {
