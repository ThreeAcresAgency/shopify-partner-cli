...
import {readStores, writeStores, fuzzySearchStores, type Merchant} from '../lib/store-manager.js';
...
export default class Remove extends Command {
  ...
  async run(): Promise<void> {
    ...
    let merchantsToRemove: Merchant[];
    ...
    // Remove merchant
    const remainingMerchants = allMerchants.filter(
      (m) => m.id !== merchantToRemove.id
    );
